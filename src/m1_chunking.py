...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        current_child = ""
        for w in words:
            if current_child and len(current_child) + 1 + len(w) > child_size:
                children.append(Chunk(text=current_child,
                                       metadata={**metadata, "chunk_type": "child", "parent_id": pid},
                                       parent_id=pid))
                current_child = w
            else:
                current_child = f"{current_child} {w}" if current_child else w
        if current_child:
            children.append(Chunk(text=current_child,
                                   metadata={**metadata, "chunk_type": "child", "parent_id": pid},
                                   parent_id=pid))
